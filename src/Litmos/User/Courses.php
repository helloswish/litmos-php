...
class Courses
{
    ...
    public function getAll(PagingSearch $ps = null)
    {
        ...
        foreach ($course_nodes as $course_node) {
            ...
            $complete  = filter_var((string)$course_node->Complete, FILTER_VALIDATE_BOOLEAN);
            $percent   = (string)$course_node->PercentageComplete;
            $startdate = !empty($course_node->StartDate) ? $course_node->StartDate : null;
            $linkid    = (string)$course_node->OriginalId;
            $course    = new Course($id, $code, $name, $active, $complete, $percent, $startdate, $linkid);
            $courses[] = $course;
        }
